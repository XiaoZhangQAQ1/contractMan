...
<head>
    ...
    <link href="<%=basePath%>css/conTemplate.css" rel="stylesheet">
    <script src="https://cdn.jsdelivr.net/npm/jquery@1.12.4/dist/jquery.min.js"></script>
</head>
<body>
<a href="../template/download?cno=${contract.cno}" style="position:fixed;top:0px;left:0px;">下载该文档</a>
	<div id="content">
		<div id="first-page">
			<p>合同编号：${contract.cno}</p>
			<p id="cno" style="display:none;">${contract.cno}</p>
			<h1>技术开发（委托）合同</h1>
			<span>项目名称：<input type="text" id="cnm" name="cnm" style="width: 400px;" value="${contract.cnm}"></span>
			<span>委托方（甲方）：<input type="text" id="fpnm" name="fpnm" value="${contract.fpnm}" style="width: 350px;" ></span>
		...
		<div id="third-page">
			<h2>技术开发（委托）合同</h2>
			<span>委托方（甲方）：<input type="text" id="fpnm1" name="fpnm1" value="${contract.fpnm}"></span>
		...
		<div id="forth-page">
			...
			<p>1．研究开发和集成经费总额为<input type="text" id="amt" name="" value="${contract.amt}"></p>
			<p>2．研究开发经费由甲方<input type="text" name="">（一次、分期或提成）支付乙方。具体支付方式和时间如下：</p>
		...
		<div style="margin-left:400px;">
			<p class="help-block" style="color:#333;">请选择您要上传的附件,支持附件格式为doc,docx</p>
			<input type="file" name="file" id="inputfile">
		</div>
		
		<button class="saveAndSub" style="background-color: #5bb85d;font-weight: bold;display: block;margin: 0 auto;margin-top: 20px;width: 150px;height: 35px;border: 1px solid #eee;color: white;font-size: 15px;font-weight: lighter;">保存并提交</button>
	</div>
</body>
<script>
$('.saveAndSub').click(function(){
	if($('#fpnm').val()!=$('#fpnm1').val()){
		alert("甲方公司名称不一致，请在确认无误后再次提交");
		return;
	}
	var amt = $('#amt').val();
	var ival = parseInt(amt);
	if(isNaN(ival)){
		alert("合同金额必须为数字");
		return;
	}
	...
	form.append("cno",cno);
	form.append("amt",amt);
	form.append("fpnm",fpnm);
	form.append("cnm",cnm);
	form.append("file",$('#inputfile')[0].files[0]);
	 $.ajax({
		url:"../crt/editOlineSub",
		type:"post",
		data:form,
		processData: false,
        contentType: false,
